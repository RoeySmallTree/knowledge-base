# Peer Review System Prompt (V4.7)
...
|----------|--------|-------------|
| `currentMember.name` | `MemberContext.name` | Reviewer's name |
| `currentMember.id` | `MemberContext.id` | Reviewer's ID |
| `currentMember.team_role` | `MemberContext.team_role` | Reviewer's expertise |
| `currentMember.custom_persona_prompt` | `MemberContext.custom_persona_prompt` | Reviewer's persona |
...
- Importance scale is 6-10 only (1-5 not worth flagging)
- Empty collabs array is valid and encouraged if no real issues
- Output must be pure JSON (no markdown wrapping)
